...
const ArticlesList: FC = observer(() => {
  const { isOpen, onClose, onOpen } = useDisclosure();

  const { articles, status, fetchArticles, totalUserArticles } = ArticlesModule;

  const isLoading = status === "loading";

  console.log(totalUserArticles);
    ...
    <SimpleGrid columns={5} gap={2} justifyItems={"stretch"} w={"100%"}>
      {isLoading &&
        new Array(4)
          .fill(1)
          .map((_, index) => (
            <Skeleton key={index} w={"100%"} h={"95px"} borderRadius={"full"} />
          ))}
      {articles.map((item, index) => (
        <Article index={index} key={item.id} item={item} />
      ))}
      {totalUserArticles < 5 && (
        <Flex
          onClick={onOpen}
